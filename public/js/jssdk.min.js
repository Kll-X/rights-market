...
//智能链接
;(function (win,doc) {
var options = {fonts: {extendedJsFonts: false}, excludes: {userAgent: true,
...
//取号地址
  ...
  optparams:{
  	...
  	isimge:false,
  	expandParams:'',
  	userInformations:getFingerPrint()
  	// str:getBrowserInfo()
  }
}
//生成uuid
...
function getFingerPrint(){
  return encodeURIComponent(base64encode(getBrowserInfos()));
}
//生成时间戳
...
function getBrowserInfo(){
  var obj = {
      ...
      //创建指纹数组
      var objs = [
      "deviceMemory","screenResolution","availableScreenResolution","timezoneOffset","timezone",
      "sessionStorage","localStorage","indexedDb","openDatabase","webglVendorAndRenderer",
      "adBlock","hasLiedLanguages","hasLiedResolution","hasLiedOs","hasLiedBrowser","touchSupport","audio"
      ]
      var str = "location:" + top['location']['href'] + "@@" + "platform:" + top['navigator']['platform'] + "@@" + "userAgent:" +  top['navigator']['userAgent']
      + "@@" + "appVersion:" + top['navigator']['appVersion'] + "@@" + "cookieEnabled:" +  top['navigator']['cookieEnabled']
      + "@@" + "cpuClass:" +   top['navigator']['cpuClass'] + "@@" + "hardwareConcurrency:" + top['navigator']['hardwareConcurrency']
      + "@@" + "language:" +  top['navigator']['language'] + "@@" + "plugins:" + top['navigator']['plugins']
      + "@@" + "availWidth:" +  top['screen']['availWidth'] + "@@" + "availHeight:" + top['navigator']['availHeight']
      + "@@" + "colorDepth:" + top['screen']['colorDepth'] + "@@" + "getTimezoneOffset:" + top['Date']['getTimezoneOffset']
      ...
      if (window.requestIdleCallback) {
          ...
          Fingerprint2.getV18(options, function (result, components) {
              for (var key in objs) {
                  for(var keys in components){
                      if (objs[key] === components[keys].key) {
                      	str += "@@" + objs[key] + ":" + components[keys].value
                      }
                  }
              }
              str += "@@phoneID:" + result;
      	  	  endTime = new Date();
      	  	  opts.optparams.expandParams  = endTime - startTime;
              opts.optparams.userInformation = encodeURIComponent(base64encode(str));
            })
          })
      } else {
          Fingerprint2.getV18(options, function (result, components) {
...
var getdisplayAddrsstartTime = new Date();
	// getdisplayAddrsstartTime = new Date();
var getdisplayAddrsendTime = "";
var getIPTime = "";
...
function updateDisplay(newAddr) {
    ...
    getdisplayAddrsendTime = new Date();
    getIPTime = getdisplayAddrsendTime - getdisplayAddrsstartTime;
...
function get_status(){
	...
	if(typeof(connection.type) == "number"){
		...
		netType = (connection.type_text === '2g') ? "1"
		: (connection.type_text === '3g')  ? "1"
		: (connection.type_text === '4g')  ? "1"
		: (connection.type_text === 'wifi')  ? "3"
		: "2"
	}else{
		...
		: (connection.type_text === '3g')  ? "1"
		: (connection.type_text === '4g')  ? "1"
		: (connection.type_text === 'cellular')  ? "1"
		: (connection.type_text === 'wifi')  ? "3"
		: "2"
	}
...
win.YDRZ = {
  ...
  getSign:function(appId,version){
      ...
      //拼接sign签名
      var signStr = appId
      + opts.optparams.msgId
      + opts.optparams.timestamp
      + opts.optparams.uuid
      + version;
      //通过业务方获取sign
      return signStr;
  ...
  getTokenInfo:function(opt){
  	  ...
  	  setTimeout(function(){
  	  //获取ip
  	  var netType = get_status();
  	  var frontPrivateIP = displayAddrs != "" ? base64encode(displayAddrs.join(",")) : "";
      //判断参数是否正确
          var options = {
              //业务方参数
              ...
              expandParams:opt.data.expandParams != "" ? opt.data.expandParams+"|frontPrivateIP=" + frontPrivateIP + "|getIPTime="+getIPTime + "|netType="+netType
              :  opt.data.expandParams+"frontPrivateIP=" + frontPrivateIP + "|getIPTime="+getIPTime + "|netType="+netType ,
              ...
              userInformation:opts.optparams.userInformations,
          }
          //校验是否启用测试线链接
          ...
          img.onload = function(){//图片加载成功,说明已经拿到号码了,这时需要向服务端去拿号
              ...
              var param = {
                  ...
                  body:{
                      expandParams:encodeURIComponent(opt.data.expandParams != '' ? opt.data.expandParams + "|gatherTime=" + opts.optparams.expandParams : opt.data.expandParams),
                      sign:options.sign,
                      uuid:options.uuid
                  }
              }
          ...
          img.onerror = function(){
              ...
              opt.error({code:"500",message:"取号失败,请检查参数是否正确",msgId:options.msgId})
              This.isLoaded = true;
              doc.body.removeChild(img);
          }
  	  },500);
